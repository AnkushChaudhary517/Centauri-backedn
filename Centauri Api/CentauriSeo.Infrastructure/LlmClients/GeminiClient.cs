...
public class GeminiClient
{
    ...
    {
        var provider = "gemini:generateContent";
    ...
    public async Task<IReadOnlyList<GeminiSentenceTag>> TagSentencesAsync(IEnumerable<Sentence> sentences, string systemRequirement)
    {
        ...
        for (int offset = 0; offset < sentenceList.Count; offset += batchSize)
        {
            ...
            if (!parsedSuccessfully)
            {
                ...
                // cache fallback chunk
                try
                {
                    var fallbackJson = JsonSerializer.Serialize(results.Skip(results.Count - chunk.Count).Take(chunk.Count).ToList());
                    await _cache.SaveAsync(chunkKey, chunkPayload, fallbackJson, provider + ":chunk:fallback");
                }
                catch { }
            }
        }
